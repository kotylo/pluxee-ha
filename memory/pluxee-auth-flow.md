...
- Integration design consequence: config flow = **manual browser login**. Generate the authorize URL with our PKCE, keep DevTools open while logging in, filter Network requests for `connect.pluxee.app`, open a matching request, and copy the `op_session*` request cookies from its Cookies tab; paste them into HA to mint tokens. The callback URL remains an alternative when it can be copied. Auth code TTL is short (~60s), so exchange immediately.
